...
class training_utils:
    ...
    def subcluster_loss_function(self,
                                 codes,
                                 logits,
                                 subresp,
                                 K,
                ...
                if codes_k.shape[0] > 0:
                    ...
                    for k_sub in range(n_sub):
                        ...
                        mus_tag = mus_sub[2 * k + k_sub].repeat(
                            codes_k.shape[0], 1)
                        loss += (r * ((torch.norm(
                            codes_k - mus_tag.to(device=self.device), dim=1))**
                                      2)).sum()
    ...
    def autoencoder_kl_dist_loss_function(z,
                                          mu,
                                          log_var,
                                          hard_assign,
                                          model_mus,
                                          ...
                                          mean=False):
        z = z.detach()
        p = torch.distributions.Normal(model_mus[hard_assign],
                                       model_std[hard_assign])
            ...
            if split_performed:
                ...
                for mu_ind in range(len(mus)):
                    if mu_ind in mus_ind_to_split:
                        ...
                        # first cluster
                        label_map[mu_ind] = (len(mus) - len(mus_ind_to_split) +
                                             count_split)
                        # list second_subcluster so we will remember to increase its label by one
                        sub_assign = resp_sub[mask_current_mu, mu_ind, 2 *
                                              mu_ind:2 * mu_ind + 2].argmax(-1)
                        inds_current_mu = mask_current_mu.nonzero(
                            as_tuple=False)
                        second_subcluster_inds = torch.cat([
                            second_subcluster_inds,
                            inds_current_mu[sub_assign == 1]
                        ])
                        count_split += 2
                    else:
                ...
                new_labels[second_subcluster_inds.clone().detach().type(
                    torch.long)] += 1
                ...
                for mu_ind in range(len(mus)):
                    if mu_ind in mus_inds_to_merge.flatten():
                        ...
                        if pairs[which_pair] == 0:
                            ...
                            label_map[mu_ind] = (
                                len(mus) - len(mus_inds_to_merge.flatten()) +
                                which_pair)
                            pairs[which_pair] += 1
                        else:
                            ...
                            which_pair_col = (
                                mus_inds_to_merge == mu_ind).nonzero(
                                    as_tuple=False)[0][1]
                            first = mus_inds_to_merge[which_pair,
                                                      (which_pair_col + 1) % 2]
    ...
    def should_init_em(self, split_performed, merge_performed,
                       previous_training_stage, current_stage):
        ...
        embeddings_changed = previous_training_stage in [
            "pretrain_ae",
            'only_ae',
            "train_ae_w_add_loss",
            "only_ae_w_cluster_loss",
            "train_together",
        ]
        if (K_changed
                or embeddings_changed) and current_stage == "only_cluster_net":
            return True
        return False

    def should_perform_em(self, current_epoch, split_performed,
                          merge_performed, previous_training_stage,
                          current_stage):
        return current_epoch > 0 and (self.should_init_em(
            split_performed, merge_performed, previous_training_stage,
            current_stage) and self.hparams.cluster_loss == "KL_GMM")

    @staticmethod
    def update_following_split(mus, mus_ind_to_split, train_resp_sub,
                               cluster_net_labels):
        label_map = {}
        count = 0
        count_split = 0
        ...
        for mu_ind in range(len(mus)):
            if mu_ind in mus_ind_to_split:
                ...
                label_map[mu_ind] = (len(mus) - len(mus_ind_to_split) +
                                     count_split)
                # list second_subcluster so we will remember to increase its label by one
                sub_assign = train_resp_sub[mask_current_mu, mu_ind, 2 *
                                            mu_ind:2 * mu_ind + 2].argmax(-1)
    ...
    @staticmethod
    def get_updated_net_labels(cluster_net_labels, split_performed,
                               merge_performed, mus, mus_ind_to_split,
                               mus_inds_to_merge, train_resp_sub):
        """ Compute the updated net labels if a split/merge has occured in this epoch
        """
        if split_performed:
            return training_utils.update_following_split(
                mus, mus_ind_to_split, train_resp_sub, cluster_net_labels)
        elif merge_performed:
            return training_utils.update_following_merge(
                mus, mus_inds_to_merge, cluster_net_labels)
    ...
    def save_cluster_examples(self,
                              logits,
                              x_for_vis,
                              y,
                              epoch,
        ...
        for k in range(K):
            ...
            if num_imgs > 0:
                ...
                save_image(
                    grid,
                    f"{self.hparams.dataset}_imgs/clusternet{init_num}_epoch{epoch}_clus{k}.jpeg"
                )
    ...
    def save_batch_of_images(self, x_for_vis, nrow=8, rows=5):
        ...
        save_image(
            grid,
            f"{self.hparams.dataset}_imgs/{self.hparams.dataset}_grid.jpeg")
